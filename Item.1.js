...
Items.animals = {
    ...
    snake: Item.make("snake", "It's a snake", function(){
        var item = Animals.snake(100,700);
        item.bodies.forEach(body => {
            body.parentItem = item;
            makeFood(body, 10);
            //body.heavy = true;
...
Items.toys = {
    ragdoll: Item.make("ragdoll", "It's a ragdoll", function(){
        var item = ragdoll(1040/2,1200/2,0.5/2);
        item.bodies.forEach(body => {
            body.parentItem = item;
            body.heavy = true;
            //Matter.Body.setMass(body, 0.1);
    ...
    ball: Item.make("ball", "ball", function(){
        var item = Matter.Bodies.circle(100 + Math.random()*20, 900/2, 20/2);
        item.isItem = true;
...
Items.food = {
    cookie: Item.make("cookie", "It's a cookie", function(){
        var item = Matter.Bodies.circle(100 + Math.random()*20, 900/2, 12);
        item.isItem = true;
        item.calories = 10;
        item.originalSize = 1;
       // item.collisionFilter.mask = currentCategory;
